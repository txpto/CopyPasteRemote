...
- **Enviar (push)**: coge lo que tengas copiado y lo deja en el buzón de la máquina
  destino. Atajo por defecto: **`Ctrl+Shift+F<N>`** (N = número de la máquina destino).
- **Pegar (pull)**: recoge lo que haya en un buzón, lo pone en tu portapapeles y lo
  ...
  - **`Ctrl+Shift+0`** → pega lo de **mi** buzón (el de esta máquina).
...
2. Copia el texto (`Ctrl+C`) y pulsa **`Ctrl+Shift+F2`**. (El cliente envía lo que
   haya en el portapapeles; activa `copy_before_send` si quieres el `Ctrl+C` automático.)
3. Ve a la **máquina 2** y pulsa **`Ctrl+Shift+2`** (o `Ctrl+Shift+0`). El texto se pega.

### 2.2 Enviar archivos o carpetas

1. En el **Explorador** de la máquina origen, selecciona archivos y/o carpetas y
   cópialos (`Ctrl+C`).
2. Pulsa **`Ctrl+Shift+F<N>`** (máquina destino).
...
| `copy_before_send` | `false` | Hacer `Ctrl+C` automático antes de enviar. |
| `notifications` | `true` | Mostrar avisos en la bandeja. |
| `prefetch` | `true` | Pre-descargar el contenido al recibir aviso. |
| `pull_own_hotkey` | `ctrl+shift+0` | Atajo para pegar de mi buzón. |
| `push_hotkeys` | `ctrl+shift+f1..9` | Mapa `slot → atajo` para enviar. |
...
`ctrl+alt+p`, `ctrl+shift+f1`, `ctrl+alt+shift+c`, `win+v` (evítalo, lo usa Windows).

Ejemplo: cambiar el envío a la máquina 2 a `Ctrl+Shift+F2` y añadir un buzón 4:

```json
"push_hotkeys": { "1": "ctrl+shift+f1", "2": "ctrl+shift+f2", "4": "ctrl+shift+f4" },
...
| "Nothing to copy" al enviar | El portapapeles estaba vacío. Copia algo (`Ctrl+C`) antes de enviar (o activa `copy_before_send`). |
...
**¿Puedo pegar en varias máquinas a la vez?** Envía a cada buzón (`Ctrl+Shift+F<N>`) y
